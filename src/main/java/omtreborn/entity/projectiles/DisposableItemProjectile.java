...
public class DisposableItemProjectile extends TurretProjectile {

    @Nullable private ItemEntity itemBound;
    private boolean spawned = false;

    public DisposableItemProjectile(EntityType<? extends DisposableItemProjectile> type, Level level) {
        super(type, level);
        this.gravity = 0.03f;
    }

    public DisposableItemProjectile(Level level, TurretBaseBlockEntity base, ItemStack ammo) {
        this(ModEntities.DISPOSABLE_ITEM_PROJECTILE.get(), level);
        this.ammo = ammo.copy();
        initFromBase(base);
    }
    ...
    public void onHitEntity(Entity entity) {
        ...
        float damage = OMTConfig.TURRETS.disposable_turret.baseDamage.get();
    ...
    public double getDamageAmpBonus() {
        return OMTConfig.TURRETS.disposable_turret.damageAmp.get();
    }
}
